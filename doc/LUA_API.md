...
bin\GoRunner.exe -f bin\script\x.script  # 回放录制脚本
```
...
**返回**：同 `MatchTemplate`（字段一致），失败为 `nil, 错误信息`
...
| `kd` / `ku` | 键盘按下 / 弹起 | `key` |
| `md` / `mu` | 鼠标按下 / 弹起 | `btn`(`left`/`right`/`center`),`x`,`y` |
| `mw` | 滚轮 | `roll` |
...
bin\GoPacker.exe script\a.script            # 打包为单文件 EXE
```
...
| **`score` 越大越好？** | 相反。用的是归一化平方差，**`score` 越接近 0 越匹配**（0 = 完全一致）。 |
